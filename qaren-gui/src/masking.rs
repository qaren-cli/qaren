...
//! presentations. Masking is a **presentation-layer** concern — patch
//! files always contain the actual values.
//!
//! This module is intentionally decoupled from `qaren-cli/src/masking.rs`
//! to maintain strict crate separation, but uses the same keyword list
//! and logic.
...
const SECRET_KEYWORDS: &[&str] = &[
    ...
    // Infrastructure and DB connection strings
    "connection_string", "conn_str", "url", "dsn",
    "redis", "rabbit", "amqp", "postgres", "mongo", "db",
];
...
pub fn mask_value<'a>(key: &str, value: &'a str, show_secrets: bool) -> &'a str {
...
mod tests {
    ...
    #[test]
    fn test_mask_value_empty_value() {
        assert_eq!(
            mask_value("API_KEY", "", false),
            "***MASKED***"
        );
    }
}
